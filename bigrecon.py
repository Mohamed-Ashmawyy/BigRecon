...
def get_subdomains_amass(domain):
    if not check_tool("amass"):
        ...
        return set()
    logger.info(f"{Colors.BLUE}[*] Running amass for {domain} (this may take a while)...{Colors.END}")

    with tempfile.NamedTemporaryFile(delete=False, suffix=".txt") as tmp:
        output_file = tmp.name

    try:
        # amass can be slow; give it extra time
        run_command(f"amass enum -passive -d {domain} -o {output_file}", timeout=600)
        subs = set()
        if os.path.exists(output_file):
            with open(output_file, 'r') as f:
                subs = {line.strip() for line in f if line.strip()}
