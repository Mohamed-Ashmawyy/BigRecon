...
def filter_live_httpx(subdomains_file, output_file ):
    ...
    # Added more flags for better data extraction
    cmd = f"httpx -l {subdomains_file} -silent -sc -cl -ip -td -server -title -json -o {json_output}"
    run_command(cmd )
        ...
        with open(json_output, 'r') as f_in, open(output_file, 'w') as f_out:
            ...
            for line in f_in:
                try:
                    ...
                    # Check multiple fields for server name
...
def main():
    ...
    parser.add_argument("-o", "--output", default="bigrecon_results.txt")
